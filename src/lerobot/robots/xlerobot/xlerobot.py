...
class XLerobot(Robot):
    ...
    def _cameras_ft(self) -> dict[str, tuple]:
        return {
            cam: (self.config.cameras[cam].height, self.config.cameras[cam].width, 3) for cam in
            self.cameras
        }
        ...
        if self.calibration_fpath.is_file():
            ...
            if user_input.strip().lower() != "c":
                ...
                try:
                    ...
                    self.bus1.calibration = {k: v for k, v in self.calibration.items() if
                                             k in self.bus1.motors}
                    self.bus2.calibration = {k: v for k, v in self.calibration.items() if
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        left_arm_pos = {k: v for k, v in action.items() if
                        k.startswith("left_arm_") and k.endswith(".pos")}
        ...
        if self.config.max_relative_target is not None:
            ...
            safe_goal_pos = ensure_safe_goal_position(goal_present_pos,
