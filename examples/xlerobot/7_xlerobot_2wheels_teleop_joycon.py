...
class FixedAxesJoyconRobotics(JoyconRobotics):
    ...
    def common_update(self):
        ...
        if joycon_stick_v > joycon_stick_v_threshold + self.joycon_stick_v_0:
            self.position[0] += speed_scale * (
                        joycon_stick_v - self.joycon_stick_v_0) / joycon_stick_v_range * \
                                self.dof_speed[0] * self.direction_reverse[0] * math.cos(pitch)
            self.position[2] += speed_scale * (
                        joycon_stick_v - self.joycon_stick_v_0) / joycon_stick_v_range * \
                                self.dof_speed[1] * self.direction_reverse[1] * math.sin(pitch)
        ...
        if joycon_stick_h > joycon_stick_h_threshold + self.joycon_stick_h_0:
            self.position[1] += speed_scale * (
                        joycon_stick_h - self.joycon_stick_h_0) / joycon_stick_h_range * \
                                self.dof_speed[1] * self.direction_reverse[1]
        ...
        if gripper_button_pressed and self.last_gripper_button_state == 0:
            ...
            print(
                f"[GRIPPER] Direction changed to: {'Open' if self.gripper_direction == 1 else 'Close'}")
...
class SimpleTeleopArm:
    ...
    def handle_joycon_input(self, joycon_pose, gripper_state):
        ...
        self.target_positions["wrist_flex"] = -self.target_positions["shoulder_lift"] - \
                                              self.target_positions["elbow_flex"] + pitch
            ...
            for key in base_action:
                if 'vel' in key:
                    ...
                    if self.current_speed > 0.01 and abs(base_action[key]) < MIN_VELOCITY_THRESHOLD:
                        ...
                        base_action[
                            key] = MIN_VELOCITY_THRESHOLD if original_value > 0 else -MIN_VELOCITY_THRESHOLD
...
def main():
    ...
    robot_config = XLerobot2WheelsConfig(
        id="my_xlerobot_2wheels_lab")  # Can be modified to your robot ID
    ...
    for i, level in enumerate(robot.speed_levels):
        print(
            f"      Level {i + 1}: Linear {level['linear']:.1f}m/s, Angular {level['angular']:.0f}°/s")
    ...
    try:
        while True:
            ...
            print(
                f"pose_right: {pose_right}, gripper_right: {gripper_right}, control_button_right: {control_button_right}")
            pose_left, gripper_left, control_button_left = joycon_left.get_control()
            print(
                f"pose_left: {pose_left}, gripper_left: {gripper_left}, control_button_left: {control_button_left}")
