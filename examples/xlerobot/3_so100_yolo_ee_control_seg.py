...
def video_stream_loop(model, cap, target_objects=None):
    ...
    while True:
        try:
            ...
            annotated_frame = cv2.cvtColor(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY),
                                           cv2.COLOR_GRAY2BGR)
...
def p_control_loop(
        robot, keyboard, target_positions, start_positions, current_x, current_y, kp=0.5,
        control_freq=50
):
    """
                ...
                for key, value in keyboard_action.items():
                    ...
                    if key in joint_controls:
                        ...
                        if joint_name in target_positions:
                            ...
                            print(
                                f"Update target position {joint_name}: {current_target} -> {new_target}")
...
def main():
    ...
    try:
        ...
        target_input = input(
            "Enter additional objects to detect (separate multiple objects with commas, e.g., bottle,cup,mouse): ").strip()
        ...
        print(
            f"  * '{DEFAULT_OBJECT}' = Blue {DEFAULT_OBJECT_COLOR} [confidence ≥ {DEFAULT_OBJECT_CONFIDENCE}]")
        print(
            f"  * Target objects = Red {TARGET_OBJECT_COLOR} [confidence ≥ {TARGET_OBJECT_CONFIDENCE}]")
        ...
        video_thread = threading.Thread(target=video_stream_loop, args=(model, cap, target_objects),
                                        daemon=True)
        video_thread.start()

        # Start keyboard control loop (main thread)
        p_control_loop(robot, keyboard, target_positions, start_positions, current_x, current_y,
