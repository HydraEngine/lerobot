...
class JoyCon:
    ...
    def set_gyro_calibration(self, offset_xyz=None, coeff_xyz=None):
        ...
        if coeff_xyz:
            cx, cy, cz = coeff_xyz
            self._GYRO_COEFF_X = 0x343b / cx if (
                        cx != 0x343b and cx != 0 and self.calibrate_value) else 1
            self._GYRO_COEFF_Y = 0x343b / cy if (
                        cy != 0x343b and cy != 0 and self.calibrate_value) else 1
            self._GYRO_COEFF_Z = 0x343b / cz if (
                        cz != 0x343b and cz != 0 and self.calibrate_value) else 1
