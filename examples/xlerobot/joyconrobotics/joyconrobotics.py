...
class AttitudeEstimator:
    ...
    def update(self, gyro_in_rad, accel_in_g):
        ...
        if self.pitch_rad_T != -1:
            self.pitch = self.pitch_rad_T if self.pitch > self.pitch_rad_T else (
                -self.pitch_rad_T if self.pitch < -self.pitch_rad_T else self.pitch)

        if self.roll_rad_T != -1:
            self.roll = self.roll_rad_T if self.roll > self.roll_rad_T else (
                -self.roll_rad_T if self.roll < -self.roll_rad_T else self.roll)

        if self.yaw_rad_T != -1:
            self.yaw = self.yaw_rad_T if self.yaw > self.yaw_rad_T else (
                -self.yaw_rad_T if self.yaw < -self.yaw_rad_T else self.yaw)
...
class JoyconRobotics:
    def __init__(self,
                 ...
                 # ZR to toggle gripper state is common for lerobot, ARX ARM and VixperX. But for UR, Sawyer and panda you could try this. ZR to go down and stick button to toggle gripper
                 lowpassfilter_alpha_rate=0.05,
                 ):
    ...
    def check_limits_position(self):
        for i in range(3):
            self.position[i] = self.glimit[0][i] if self.position[i] < self.glimit[0][i] else (
                self.glimit[1][i] if self.position[i] > self.glimit[1][i] else self.position[i])

    def check_limits_orientation(self):
        for i in range(3):
            self.orientation_rad[i] = self.glimit[0][3 + i] if self.orientation_rad[i] < \
                                                               self.glimit[0][3 + i] else (
                self.glimit[1][3 + i] if self.orientation_rad[i] > self.glimit[1][3 + i] else
    ...
    def common_update(self):
        ...
        for event_type, status in self.button.events():
            ...
            elif ((self.joycon.is_right() and event_type == 'zr') or (
                    self.joycon.is_left() and event_type == 'zl')) and not self.change_down_to_gripper:
                self.gripper_toggle_button = status
            elif ((self.joycon.is_right() and event_type == 'stick_r_btn') or (
                    self.joycon.is_left() and event_type == 'stick_l_btn')) and self.change_down_to_gripper:
                self.gripper_toggle_button = status
    ...
    def get_orientation(self):  # euler_rad, euler_deg, quaternion,
        self.orientation_rad = self.orientation_sensor.update(self.gyro.gyro_in_rad[0],
                                                              self.gyro.accel_in_g[0])
        ...
        for i in range(3):  # deal with offset and reverse
            self.orientation_rad[i] = (self.orientation_rad[i] + self.offset_euler_rad[i]) * \
