    ...
    if not results or not hasattr(results[0], 'boxes') or not results[0].boxes:
        ...
        for box in results[0].boxes:
            ...
            if label in target_objects:
                ...
                print(
                    f"{label.capitalize()} center offset: dx={dx}, dy={dy} -> dc_y={d_current_y}, pan: {target_positions['shoulder_pan']:.2f}, y: {current_y:.3f}, joint2: {joint2_target:.2f}, joint3: {joint3_target:.2f}")
...
def p_control_loop(robot, keyboard, target_positions, start_positions, current_x, current_y, kp=0.5,
                   control_freq=50, model=None, cap=None, vision_mode=False,
                   target_objects=["mouse"]):
    """
    P control loop
    ...
    while True:
        try:
            ...
            if 'shoulder_lift' in target_positions and 'elbow_flex' in target_positions:
                target_positions['wrist_flex'] = - target_positions['shoulder_lift'] - \
                                                 target_positions['elbow_flex'] + pitch
...
def main():
    ...
    try:
        ...
        p_control_loop(robot, keyboard, target_positions, start_positions, current_x, current_y,
                       kp=0.5, control_freq=50, model=model, cap=cap, vision_mode=vision_mode,
