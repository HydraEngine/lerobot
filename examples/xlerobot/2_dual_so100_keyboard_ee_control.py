        ...
        for arm_name, robot in robots.items():
            ...
            for key, value in current_obs[arm_name].items():
                if key.endswith('.pos'):
                    motor_name = key.removesuffix('.pos')
                    current_positions[arm_name][
                        motor_name] = value  # Don't apply calibration coefficients
...
def p_control_loop(robots, keyboard, target_positions, start_positions, current_positions, kp=0.5,
                   control_freq=50):
    """
    P control loop
    ...
    while True:
        try:
                ...
                for key, value in keyboard_action.items():
                    ...
                    if key in arm1_joint_controls:
                        ...
                        if joint_name in target_positions['arm1']:
                            ...
                            print(
                                f"First arm update target position {joint_name}: {current_target} -> {new_target}")
                    ...
                    # Second arm joint control
                        ...
                        if joint_name in target_positions['arm2']:
                            ...
                            print(
                                f"Second arm update target position {joint_name}: {current_target} -> {new_target}")
                    ...
                    # First arm xy control
                        ...
                        if coord == 'x':
                            ...
                            print(
                                f"First arm update x coordinate: {current_positions['arm1']['x']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}")
                        elif coord == 'y':
                            ...
                            target_positions['arm1']['elbow_flex'] = joint3_target
                            print(
                                f"First arm update y coordinate: {current_positions['arm1']['y']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}")
                    ...
                    # Second arm xy control
                        ...
                        if coord == 'x':
                            ...
                            print(
                                f"Second arm update x coordinate: {current_positions['arm2']['x']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}")
                        elif coord == 'y':
                            ...
                            target_positions['arm2']['elbow_flex'] = joint3_target
                            print(
                                f"Second arm update y coordinate: {current_positions['arm2']['y']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}")

            # Apply pitch adjustment to wrist_flex for each robot arm
            for arm_name in ['arm1', 'arm2']:
                if 'shoulder_lift' in target_positions[arm_name] and 'elbow_flex' in \
                        target_positions[arm_name]:
                    target_positions[arm_name]['wrist_flex'] = - target_positions[arm_name][
                        'shoulder_lift'] - target_positions[arm_name]['elbow_flex'] + pitch[
                                                                   arm_name]
            ...
            if p_control_loop.step_counter % 100 == 0:
                print(
                    f"Current pitch adjustment: arm1={pitch['arm1']:.3f}, arm2={pitch['arm2']:.3f}")
    ...
    try:
        ...
        while True:
            calibrate_choice = input(
                "Do you want to recalibrate the robots? (y/n): ").strip().lower()
        ...
        for arm_name, robot in robots.items():
            ...
            for key, value in start_obs.items():
                if key.endswith('.pos'):
                    motor_name = key.removesuffix('.pos')
                    start_positions[arm_name][motor_name] = int(
                        value)  # Don't apply calibration coefficients
        ...
        print(
            f"Initialize dual arm end effector positions: arm1=({x0:.4f}, {y0:.4f}), arm2=({x0:.4f}, {y0:.4f})")
        ...
        p_control_loop(robots, keyboard, target_positions, start_positions, current_positions,
