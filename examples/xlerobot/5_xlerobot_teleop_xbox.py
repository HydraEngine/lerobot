...
class SimpleTeleopArm:
    ...
    def handle_keys(self, key_state):
        ...
        if key_state.get('gripper+'):
            ...
            print(
                f"[{self.prefix}] gripper: auto-opening to {self.target_positions['gripper']:.1f}")
...
def get_xbox_key_state(joystick, keymap):
    ...
    for action, control in keymap.items():
        ...
        # Left stick controls (when not pressed)
            ...
            state[action] = (not left_stick_pressed) and (not lb_pressed) and (
                        axes[1] < -0.5) if len(axes) > 1 else False
        elif control == 'left_stick_down':
            state[action] = (not left_stick_pressed) and (not lb_pressed) and (
                        axes[1] > 0.5) if len(axes) > 1 else False
        elif control == 'left_stick_left':
            state[action] = (not left_stick_pressed) and (not lb_pressed) and (
                        axes[0] < -0.5) if len(axes) > 0 else False
        elif control == 'left_stick_right':
            state[action] = (not left_stick_pressed) and (not lb_pressed) and (
                        axes[0] > 0.5) if len(axes) > 0 else False
        # Right stick controls (when not pressed) - Fixed axis mapping
        elif control == 'right_stick_up':
            state[action] = (not right_stick_pressed) and (not rb_pressed) and (
                        axes[4] < -0.5) if len(axes) > 4 else False
        elif control == 'right_stick_down':
            state[action] = (not right_stick_pressed) and (not rb_pressed) and (
                        axes[4] > 0.5) if len(axes) > 4 else False
        elif control == 'right_stick_left':
            state[action] = (not right_stick_pressed) and (not rb_pressed) and (
                        axes[3] < -0.5) if len(axes) > 3 else False
        elif control == 'right_stick_right':
            state[action] = (not right_stick_pressed) and (not rb_pressed) and (
                        axes[3] > 0.5) if len(axes) > 3 else False
        # Left stick pressed controls
        elif control == 'left_stick_pressed_right':
            state[action] = left_stick_pressed and (not lb_pressed) and (axes[0] > 0.5) if len(
                axes) > 0 else False
        ...
        # Right stick pressed controls - Fixed axis mapping
            ...
            state[action] = right_stick_pressed and (not rb_pressed) and (axes[3] > 0.5) if len(
                axes) > 3 else False
        ...
        # LB pressed controls (only when stick is moved)
            ...
            state[action] = lb_pressed and (abs(axes[1]) > 0.5) and (axes[1] < -0.5) if len(
                axes) > 1 else False
        elif control == 'lb_down':
            state[action] = lb_pressed and (abs(axes[1]) > 0.5) and (axes[1] > 0.5) if len(
                axes) > 1 else False
        elif control == 'lb_right':
            state[action] = lb_pressed and (abs(axes[0]) > 0.5) and (axes[0] > 0.5) if len(
                axes) > 0 else False
        ...
        # RB pressed controls (only when stick is moved)
            ...
            state[action] = rb_pressed and (abs(axes[4]) > 0.5) and (axes[4] < -0.5) if len(
                axes) > 4 else False
        elif control == 'rb_down':
            state[action] = rb_pressed and (abs(axes[4]) > 0.5) and (axes[4] > 0.5) if len(
                axes) > 4 else False
        elif control == 'rb_right':
            state[action] = rb_pressed and (abs(axes[3]) > 0.5) and (axes[3] > 0.5) if len(
                axes) > 3 else False
