...
class SimpleTeleopArm:
    ...
    def handle_vr_input(self, vr_goal, gripper_state):
        ...
        try:
            joint2_target, joint3_target = self.kinematics.inverse_kinematics(self.current_x,
                                                                              self.current_y)
            ...
            self.target_positions["shoulder_lift"] = (1 - alpha) * self.target_positions.get(
                "shoulder_lift", 0.0) + alpha * joint2_target
            self.target_positions["elbow_flex"] = (1 - alpha) * self.target_positions.get(
                "elbow_flex", 0.0) + alpha * joint3_target
...
def main():
    ...
    try:
        ...
        vr_thread = threading.Thread(target=lambda: asyncio.run(vr_monitor.start_monitoring()),
                                     daemon=True)
        vr_thread.start()
        ...
        left_arm = SimpleTeleopArm(LEFT_JOINT_MAP, obs, kin_left,
                                   prefix="left") if ENABLE_LEFT_HAND else None
        ...
        dataset_saving_thread = threading.Thread(target=saving_dataset_worker, args=thread_args,
                                                 daemon=False)
        ...
        while not shutdown_event.is_set():
            ...
            left_action = left_arm.p_control_action(robot) if (
                        ENABLE_LEFT_HAND and left_arm) else {}
            right_action = right_arm.p_control_action(robot)
            head_action = head_control.p_control_action(robot) if (
                        ENABLE_HEAD and head_control) else {}
            ...
            lerobot_frame = {
                ...
                'observation.state': np.array(list(robot.get_observation().values())[6:12],
                                              dtype=np.float32),
