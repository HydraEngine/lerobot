    ...
    def common_update(self):
        ...
        if joycon_stick_v > joycon_stick_v_threshold + joycon_stick_v_0:
            self.position[0] += speed_scale * (
                        joycon_stick_v - joycon_stick_v_0) / joycon_stick_v_range * self.dof_speed[
                                    0] * self.direction_reverse[0] * self.direction_vector[0]
            self.position[2] += speed_scale * (
                        joycon_stick_v - joycon_stick_v_0) / joycon_stick_v_range * self.dof_speed[
                                    1] * self.direction_reverse[1] * self.direction_vector[2]
        ...
        if joycon_stick_h > joycon_stick_h_threshold + joycon_stick_h_0:
            self.position[1] += speed_scale * (
                        joycon_stick_h - joycon_stick_h_0) / joycon_stick_h_range * self.dof_speed[
                                    1] * self.direction_reverse[1]
...
def p_control_loop(
        robot, keyboard, target_positions, start_positions, current_x, current_y,
        joyconrobotics_right, kp=0.5, control_freq=50
):
    """
    P control loop
    ...
    try:
        ...
        while True:
            calibrate_choice = input(
                "Do you want to recalibrate the robot? (y/n): ").strip().lower()
